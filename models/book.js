const db = require("../db");
const createBookSchema = require('../schemas/createBookSchema.json');

/** Collection of related methods for books. */

class Book {
  /** given an isbn, return book data with that isbn:
   *
   * => {isbn, amazon_url, author, language, pages, publisher, title, year}
   *
   **/

  static async findOne(isbn) {
    const bookRes = await db.query(
        `SELECT isbn,
                amazon_url,
                author,
                language,
                pages,
                publisher,
                title,
                year
            FROM books 
            WHERE isbn = $1`, [isbn]);

    if (bookRes.rows.length === 0) {
      throw { message: `There is no book with an isbn '${isbn}`, status: 404 }
    }

    return bookRes.rows[0];
  }

  /** Return array of book data:
   *
   * => [ {isbn, amazon_url, author, language,
   *       pages, publisher, title, year}, ... ]
   *
   * */

  static async findAll() {
    const booksRes = await db.query(
        `SELECT isbn,
                amazon_url,
                author,
                language,
                pages,
                publisher,
                title,
                year
            FROM books 
            ORDER BY title`);

    return booksRes.rows;
  }

  /** create book in database from data, return book data:
   *
   * {isbn, amazon_url, author, language, pages, publisher, title, year}
   *
   * => {isbn, amazon_url, author, language, pages, publisher, title, year}
   *
   * */

  static async create(data) {
    
    const result = await db.query(
      `INSERT INTO books (
            isbn,
            amazon_url,
            author,
            language,
            pages,
            publisher,
            title,
            year) 
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8) 
         RETURNING isbn,
                   amazon_url,
                   author,
                   language,
                   pages,
                   publisher,
                   title,
                   year`,
      [
        data.isbn,
        data.amazon_url,
        data.author,
        data.language,
        data.pages,
        data.publisher,
        data.title,
        data.year
      ]
    );

    return result.rows[0];
  }

  /** Update data with matching ID to data, return updated book.

   * {isbn, amazon_url, author, language, pages, publisher, title, year}
   *
   * => {isbn, amazon_url, author, language, pages, publisher, title, year}
   *
   * */

  static async update(isbn, data) {
    // Step 1: Retrieve the current book details from the database
    const currentBook = await db.query(`SELECT * FROM books WHERE isbn = $1`, [isbn]);
  
    // Step 2: Check if the book exists
    if (currentBook.rows.length === 0) {
      throw { message: `There is no book with an isbn '${isbn}'`, status: 404 };
    }
  
    // Step 3: Prepare the updated fields, using existing values if not provided
    const updatedBook = {
      amazon_url: data.amazon_url !== undefined ? data.amazon_url : currentBook.rows[0].amazon_url,
      author: data.author !== undefined ? data.author : currentBook.rows[0].author,
      language: data.language !== undefined ? data.language : currentBook.rows[0].language,
      pages: data.pages !== undefined ? data.pages : currentBook.rows[0].pages,
      publisher: data.publisher !== undefined ? data.publisher : currentBook.rows[0].publisher,
      title: data.title !== undefined ? data.title : currentBook.rows[0].title,
      year: data.year !== undefined ? data.year : currentBook.rows[0].year
    };
  
    // Step 4: Perform the update query with only the updated fields
    const result = await db.query(
      `UPDATE books SET 
            amazon_url = $1,
            author = $2,
            language = $3,
            pages = $4,
            publisher = $5,
            title = $6,
            year = $7
       WHERE isbn = $8
       RETURNING isbn,
                 amazon_url,
                 author,
                 language,
                 pages,
                 publisher,
                 title,
                 year`,
      [
        updatedBook.amazon_url,
        updatedBook.author,
        updatedBook.language,
        updatedBook.pages,
        updatedBook.publisher,
        updatedBook.title,
        updatedBook.year,
        isbn
      ]
    );
  
    return result.rows[0]; // Return the updated book
  }

  /** remove book with matching isbn. Returns undefined. */

  static async remove(isbn) {
    const result = await db.query(
      `DELETE FROM books 
         WHERE isbn = $1 
         RETURNING isbn`,
        [isbn]);

    if (result.rows.length === 0) {
      throw { message: `There is no book with an isbn '${isbn}`, status: 404 }
    }
  }
}


module.exports = Book;
